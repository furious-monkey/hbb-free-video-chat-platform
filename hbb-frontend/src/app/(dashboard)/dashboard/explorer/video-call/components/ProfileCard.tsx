// dashboard/explorer/video-call/components/ProfileCard.tsx - Enhanced with live video stream
"use client";
...
export default function ProfileCard({
  ...
  const formatCallRate = (rate?: number) => {
    if (!rate) return 'Free';
    return `$${rate}/min`;
  };
  ...
  const getDisplayName = () => {
    ...
    return influencerName || explorerName || 'User';
  };
        ...
        <div className={`w-full h-1/4 bg-black bg-opacity-80 text-white p-2 lg:p-3 transition-all duration-300 ${
          ...
          <div className="flex justify-between items-center text-xs lg:text-sm">
            <div>
              <p className="text-gray-300">
                {isExplorer ? 'Rate' : 'Status'}
              </p>
              <p className="font-semibold">
                {isExplorer ? formatCallRate(callRate) : 'Connected'}
              </p>
            </div>
          ...
          {showDetails && (
            <div className="mt-2 lg:mt-3 pt-2 lg:pt-3 border-t border-gray-600 space-y-2">
              {/* Show earnings for influencer view */}
              {isInfluencer && earnings !== undefined && (
                <div className="flex justify-between items-center text-xs lg:text-sm">
                  <span className="text-gray-300">Session Spent</span>
                  <span className="text-blue-400 font-semibold">{formatEarnings(earnings)}</span>
                </div>
              )}
              
              {/* Show different stats for explorer view */}
              {isExplorer && (
                <div className="flex justify-between items-center text-xs lg:text-sm">
              ...
              <div className="grid grid-cols-2 gap-1 lg:gap-2 mt-2">
                ...
                <button className="bg-purple-500 hover:bg-purple-600 text-white text-xs py-1 px-2 rounded transition-colors">
                  {isExplorer ? 'Follow' : 'Message'}
                </button>
              </div>
            </div>
          )}
